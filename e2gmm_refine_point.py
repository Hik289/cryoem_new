...
def main():
    ...
    parser.add_argument(
        "--chunk",
        type=str,
        help="In case of memory exhaustion, particles can be trained in chunks (similar to a batch). eg - 0,10 will process the first of 10 chunks. Be sure to store and restore the encoder and decoder if using this mode. Must be run sequentially 0-(n-1) without parallelism.",
        default=None,
    )
    parser.add_argument(
        "--sym", type=str, help="symmetry. currently only support c and d", default="c1"
    )
    parser.add_argument(
        "--model", type=str, help="load from an existing model file", default=""
    )
    ...
    parser.add_argument(
        "--decoderin",
        type=str,
        help="Rather than initializing the decoder from a model, read an existing trained decoder",
        default="",
    )
    parser.add_argument(
        "--decoderout",
        type=str,
        help="Save the trained decoder model. Filename should be .h5",
        default=None,
    )
    parser.add_argument(
        "--encoderin",
        type=str,
        help="Rather than initializing the encoder from scratch, read an existing trained encoder",
        default=None,
    )
    parser.add_argument(
        "--encoderout",
        type=str,
        help="Save the trained encoder model. Filename should be .h5",
        default=None,
    )
    parser.add_argument(
        "--projs",
    ...
    parser.add_argument(
        "--ptclsin", type=str, help="particles input for alignment", default=""
    )
    parser.add_argument(
        "--ptclsout", type=str, help="aligned particle output", default=""
    )
    parser.add_argument(
        "--ptclsclip",
        type=int,
        help="clip particles to specified box size before use",
        default=-1,
    )
    parser.add_argument(
        "--learnrate",
    ...
    parser.add_argument(
        "--modelreg",
        type=float,
        help="regularizer for for Gaussian positions based on the starting model, ie the result will be biased towards the starting model when training the decoder (0-1 typ). Default 0",
        default=0,
    )
    parser.add_argument(
        "--ampreg",
    ...
    parser.add_argument(
        "--npts", type=int, help="number of points to initialize. ", default=-1
    )
    parser.add_argument("--batchsz", type=int, help="batch size", default=256)
    parser.add_argument(
        "--minressz",
        type=int,
        help="Fourier diameter associated with minimum resolution to consider. ",
        default=4,
    )
    parser.add_argument(
        "--maxboxsz",
    ...
    parser.add_argument(
        "--align", action="store_true", default=False, help="align particles."
    )
    parser.add_argument(
        "--heter", action="store_true", default=False, help="heterogeneity analysis."
    )
    parser.add_argument(
        "--decoderentropy",
        action="store_true",
        default=False,
        help="This will train some entropy into the decoder using particles to reduce vanishing gradient problems",
    )
    parser.add_argument(
        "--perturb",
        type=float,
        default=0.1,
        help="Relative perturbation level to apply in each iteration during --heter training. Default = 0.1, decrease if models are too disordered",
    )
    parser.add_argument(
        "--conv",
        action="store_true",
        default=False,
        help="Use a convolutional network for heterogeneity analysis.",
    )
    parser.add_argument(
        "--fromscratch",
        action="store_true",
    ...
    parser.add_argument(
        "--ptclrepout",
        type=str,
        help="Save the per-particle representation input to the network to a file for later use",
        default="",
    )
    parser.add_argument(
        "--ptclrepin",
        type=str,
        help="Load the per-particle representation from a file rather than recomputing",
        default="",
    )
    ...
    parser.add_argument(
        "--pas",
        type=str,
        help="choose whether to adjust position, amplitude, sigma. sigma is not supported in this version of the program. use 3 digit 0/1 input. default is 110, i.e. only adjusting position and amplitude",
        default="110",
    )
    parser.add_argument(
        "--nmid",
        type=int,
        help="size of the middle layer. If model is grouped becomes nmid*ngroup",
        default=4,
    )
    parser.add_argument(
        "--mask", type=str, help="remove points outside mask", default=""
    )
    parser.add_argument(
        "--ppid",
        type=int,
    ...
    if options.chunk is not None:
        try:
            options.chunk = (
                int(options.chunk.split(",")[0]),
                int(options.chunk.split(",")[1]),
            )
        except:
    ...
    if options.model:
        if options.model.endswith(".pdb"):
            ...
            if pts.shape[1] == 5:
                ...
                pts = np.array(
                    sorted(pts, key=lambda x: x[5])
                )  # sort points by group so they can be fed into the subencoders
                grps = Counter(
                    pts[:, 5]
                )  # count elements in each group to generate correct encoder
                if options.nmid % len(grps) != 0 and options.encoderin is None:
                    error_exit(
                        f"ERROR: Size of latent space {options.nmid} must be divisible by the number of groups {len(grps)}"
                    )
                pts = pts[:, :4]
    ...
    if (not options.decoderin) and options.model and options.projs:
        ...
        for i in range(500):
            loss.append(
                gen_model.train_on_batch(
                    conf, pts.reshape(1, pts.shape[0], pts.shape[1])
                )
            )
    ...
    if options.projs:
        ...
        if options.maxres > 0:
            ...
            maxboxsz = options.maxboxsz = good_size(
                ceil(raw_boxsz * raw_apix * 2 / options.maxres) // 2 * 2
            )
            print(
                "using box size {}, max resolution {:.1f}".format(
                    maxboxsz, options.maxres
                )
            )

        data_cpx, xfsnp, grpdct = load_particles(
            options.projs, maxboxsz, preclip=options.ptclsclip, chunk=options.chunk
        )
    ...
    if options.ptclsin:
        e = EMData(
            options.ptclsin, 0
        )  # used to be header only, but inconvenient with clip possibility
        if options.ptclsclip > 0:
            e = e.get_clip(
                Region(
                    (e["nx"] - options.ptclsclip) // 2,
                    (e["ny"] - options.ptclsclip) // 2,
                    options.ptclsclip,
                    options.ptclsclip,
                )
            )
        ...
        data_cpx, xfsnp, grpdct = load_particles(
            options.ptclsin, maxboxsz, preclip=options.ptclsclip, chunk=options.chunk
        )
        apix = raw_apix * raw_boxsz / maxboxsz
        clipid = set_indices_boxsz(data_cpx[0].shape[1], apix, True)
        ...
        if options.ptclrepin:
            ...
            if not grpdct is None:
                for k in grpdct.keys():
                    ...
                    allgrds[grpdct[k]] = np.add.reduce(allgrds[grpdct[k]]) * (
                        10.0 / len(grpdct[k])
                    )  # 10 aribtrary value for test
    ...
    if options.ptclsin and options.heter:
        ...
        if options.encoderin is not None:
            encode_model = tf.keras.models.load_model(
                f"{options.encoderin}", compile=False
            )
        else:
            encode_model = build_encoder(len(pts[0]), options.nmid, grps)

        if options.decoderin:
            decode_model = tf.keras.models.load_model(
                f"{options.decoderin}", compile=False
            )
        else:
        ...
        trainset = tf.data.Dataset.from_tensor_slices(
            (allgrds[ptclidx], dcpx[0][ptclidx], dcpx[1][ptclidx], xfsnp[ptclidx])
        )
        trainset = trainset.batch(bsz)
...
def xf2pts(pts, ang):
    ...
    matrix = tf.reshape(
        matrix, shape=[-1, 3, 3]
    )  #### Here we get a batch_size x 3 x 3 matrix
...
##           sym - symmetry string
# @tf.function
def pts2img(pts, ang, params, lp=0.1, sym="c1"):
    bsz = ang.shape[0]
    ...
    for p in p0:
        ...
        imgs = tf.stack(
            [
                tf.tensor_scatter_nd_add(imgs[i], bposall[i], bampall[i])
...
def load_particles(fname, boxsz, preclip=-1, chunk=None):
    ...
    print(
        "Loading {} particles of box size {}. shrink to {}".format(nptcl, rawbox, boxsz)
    )
    ...
    if "ptcl3d_id" in hdrs[0]:
        ...
        for i, h in enumerate(hdrs):
            try:
                grpdct[h["ptcl3d_id"]].append(i)
            except:
                grpdct[h["ptcl3d_id"]] = [i]
        print(
            f"identified {len(grpdct)} 3-D particles, merging gradients over each 3-D particle"
        )
    else:
        grpdct = None
    ...
    xfsnp = np.array(
        [[x["az"], x["alt"], x["phi"], x["tx"], x["ty"]] for x in xfs], dtype=floattype
    )
    xfsnp[:, :3] = xfsnp[:, :3] * np.pi / 180.0
...
def set_indices_boxsz(boxsz, apix=0, return_freq=False):
    ...
    if return_freq:
        ...
        rings = np.zeros(
            (sz, sz // 2 + 1, sz // 2), dtype=floattype
        )  #### Fourier rings
        for i in range(sz // 2):
            rings[:, :, i] = rr == i
...
def build_encoder(ninp, nmid, grps=None):
    ...
    if grps is None:
        ...
        layers = [
            ...
            tf.keras.layers.Conv2D(
                4, 5, activation="relu", strides=(2, 2), padding="same"
            ),
            tf.keras.layers.Conv2D(
                8, 5, activation="relu", strides=(2, 2), padding="same"
            ),
            tf.keras.layers.Conv2D(
                16, 3, activation="relu", strides=(2, 2), padding="same"
            ),
            tf.keras.layers.Conv2D(
                16, 3, activation="relu", strides=(2, 2), padding="same"
            ),
            ...
            tf.keras.layers.Dense(nmid, kernel_initializer=kinit),
        ]
        ...
        for i in ngrp:
            in2s.append(
                ...
                # )(in1[:, t : t + i])
                tf.keras.layers.Dense(ss * ss, kernel_regularizer=l2)(in1[:, t : t + i])
            )
            t += i
        # Add Dropout here?
        mid = [
            # tf.keras.layers.Dense(
            # max(ngrp[i] // 8, latpergrp * 4),
            # activation="relu",
            # kernel_initializer=kinit,
            # kernel_regularizer=l2,
            # use_bias=True,
            # )(in2s[i])
            tf.keras.layers.Reshape((ss, ss, 1))(in2s[i])
            for i in range(len(ngrp))
        ]
        mid2 = [
            # tf.keras.layers.Dense(
            #     max(ngrp[i] // 32, latpergrp * 2),
            #     activation="relu",
            #     kernel_initializer=kinit,
            #     kernel_regularizer=l2,
            #     use_bias=True,
        ...
        mid6 = [tf.keras.layers.Flatten()(mid5[i])]
        mid7 = [tf.keras.layers.Dropout(0.1)(mid6[i])]
        mid8 = [tf.keras.layers.BatchNormalization()(mid7[i])]
        outs = [
            tf.keras.layers.Dense(
                latpergrp,
                kernel_regularizer=l2,
...
def build_decoder(nmid, pt):
    ...
    layer_output = tf.keras.layers.Dense(
        nout * 4, kernel_initializer=binit, activation="sigmoid", use_bias=True
    )
    ...
    #     ),
    #     tf.keras.layers.Dense(
    #         nmid * 4, activation="relu", kernel_initializer=kinit, use_bias=True
    #     ),
    #     tf.keras.layers.Dense(
    #         nmid * 8, activation="relu", kernel_initializer=kinit, use_bias=True
    #     ),
    ...
    bshift = np.array([-0.5, -0.5, -0.5, 0]).astype(floattype)
    ...
    for itr in range(options.niter):
        ...
        for pjr, pji, xf in trainset:
            ...
            with tf.GradientTape() as gt:
                ...
                if options.decoderentropy:
                    conf = tf.random.uniform(
                        (xf.shape[0], options.nmid), minval=-0.02, maxval=0.02
                    )
                ...
                if options.decoderentropy:
                    pout = gen_model(
                        conf
                        + tf.random.uniform(
                            (conf.shape[0], conf.shape[1]), minval=-0.02, maxval=0.02
                        )
                    )
                else:
                    pout = gen_model(conf)
                ...
                fval = calc_frc(
                    pj_cpx, imgs_cpx, params["rings"], minpx=options.minressz
                )
                ...
                if options.modelreg > 0:
                    ...
                    l += (
                        tf.reduce_sum((pout[0, :, :3] - pts[:, :3]) ** 2)
                        / len(pts)
                        * options.modelreg
                        * 20.0
                    )  # factor of 20 is a rough calibration relative to the dynamic training
                if itr < options.niter // 2:
                    l += std[3] * options.ampreg * options.ampreg
            # 				print(std)
            ...
            sys.stdout.write(
                "\r {}/{}\t{:.3f} ({:.3f})         ".format(len(cost), nbatch, loss, l)
            )
            sys.stdout.flush()
        sys.stdout.write("\r")

        print(
            "iter {}, loss : {:.4f} ({:.4f})         ".format(
                itr, np.mean(cost), np.mean(truecost)
            )
        )
...
def coarse_align(dcpx, pts, options):
    ...
    xfs = symmetry.gen_orientations(
        "eman", {"delta": astep, "phitoo": astep, "inc_mirror": 1}
    )
    xfs = [x.get_params("eman") for x in xfs]
    xfsnp = np.array(
        [[x["az"], x["alt"], x["phi"], x["tx"], x["ty"]] for x in xfs], dtype=floattype
    ...
    for ii in range(niter):
        ...
        for idt in range(npt):
            ...
            sys.stdout.write(
                "\r projs: {}/{}, data: {}/{}	".format(ii + 1, niter, idt + 1, npt)
            )
            sys.stdout.flush()
    xfs = []
    for ii in range(1):
...
def refine_align(dcpx, xfsnp, pts, options, lr=1e-3):
    ...
    for ptr, ptj, xf in trainset:
        ...
        for itr in range(niter):
            ...
            sys.stdout.write(
                "\r batch {}/{}, iter {}/{}, loss {:.3f}   ".format(
                    len(xfvs), nbatch, itr + 1, niter, loss
                )
            )
            sys.stdout.flush()
...
def calc_gqual(trainset, pts, params, options):
    ...
    for pjr, pji, xf in trainset:
        ...
        for k in range(pt.shape[1]):
            ...
            fval[:, k] = (
                calc_frc(pj_cpx, imgs_cpx, params["rings"], minpx=options.minressz)
                - base
            )
...
def calc_gradient(trainset, pts, params, options):
    ...
    for pjr, pji, xf in trainset:
        ...
        sys.stdout.write(
            "\r {}/{} : {:.4f}        ".format(len(allscr), nbatch, np.mean(fval))
        )
        sys.stdout.flush()
    ...
    for itr in range(options.niter):
        ...
        for grd, pjr, pji, xf in trainset:
            ...
            with tf.GradientTape() as gt:
                ...
                conf = (options.perturb * 2.0 / (itr + 2.0)) * tf.random.normal(
                    conf.shape
                ) + conf  # perturbation is reduced each iteration
            ...
            if i % 10 == 0:
                sys.stdout.write(
                    "\r {}/{}\t{:.3f}         ".format(len(cost), nbatch, loss)
                )
                sys.stdout.flush()
